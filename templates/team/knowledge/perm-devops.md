...
- Verification habits: trigger pipelines/hooks end-to-end before `[DONE]`; after any pipeline change, check `curl -s http://localhost:8000/api/webhooks/health | jq .`; secrets only in `.soren/secrets.db` or env
